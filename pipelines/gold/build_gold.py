...
def build_fact(
    ...
    co2_cols = [
        ...
        "co2", "co2_per_capita", "co2_per_capita_calc",
        "co2_intensity_gdp", "gdp", "gdp_per_capita",
        "gdp_growth_pct", "population", "share_global_co2",
    ]
    ...
    tourism_cols = [
        "country_code", "year",
        "tourist_arrivals", "tourism_receipts_usd",
        "tourist_departures", "arrivals_growth_pct",
    ...
    # JOIN secuencial
    fact = pd.merge(
        df_co2_slim, df_tourism_slim,
        on=["country_code", "year"], how="outer",
    )
    fact = pd.merge(
        fact, df_transport_slim,
        on=["country_code", "year"], how="outer",
    )
    ...
    fact.loc[mask, "co2_per_tourist"] = (
        fact.loc[mask, "co2"] * 1_000_000 / fact.loc[mask, "tourist_arrivals"]
    ).round(4)
