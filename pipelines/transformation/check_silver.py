...
for name, path in DATASETS.items():
    ...
    if missing_countries:
        ...
        print(f"  Sin ningun dato: ninguno")

    if not incomplete.empty:
        print(f"  Paises con anos faltantes:")
        for iso, count in incomplete.items():
            missing_years = sorted(
                set(range(2013, 2024)) - set(df[df["country_code"] == iso]["year"])
            )
            print(f"    {iso}: {count}/11 anos — faltan {missing_years}")
    else:
        print(f"  Cobertura de anos: completa en todos los paises")